import { PlayerWithStats } from '@game/types';
...
export const PlayerWithTooltip = ({ player }: { player: PlayerWithStats }) => {
  ...
  return (
    <HoverCard width={320} shadow="md" withArrow openDelay={100} closeDelay={100}>
      <HoverCard.Target>
        <Text className={classes.playerName} py={3} c={!player.isAlive ? 'dimmed' : ''} >{player.name}</Text>
      </HoverCard.Target>
      <HoverCard.Dropdown>
        ...
        <Center pt={'1rem'}>
          <Button size={'xs'} variant={'outline'} loading={loading && !requested} onClick={handleClick} color={'white'}>{requested ? 'Requested' : 'Send Friend Request'}</Button>
        </Center>
      </HoverCard.Dropdown>
    </HoverCard>
  )
